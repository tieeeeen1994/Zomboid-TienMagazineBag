...
function MagazineBag_MoveToBag:perform()
    ...
    if bagContainer and bagContainer:hasRoomFor(self.character, self.magazine) then
        playerInventory:DoRemoveItem(self.magazine)
        bagContainer:AddItem(self.magazine)
        sendRemoveItemFromContainer(playerInventory, self.magazine)
        sendAddItemToContainer(bagContainer, self.magazine)
    end
